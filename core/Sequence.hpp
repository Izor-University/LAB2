...
public:
    ...
    // Чисто виртуальные методы декомпозиции (должны быть в наследниках)
    virtual const T& Get(int index) const override = 0;
    virtual int GetLength() const override = 0;

    // Чисто виртуальные методы модификации (зависят от памяти)
    virtual Sequence<T>* Append(const T& item) = 0;
    virtual Sequence<T>* Prepend(const T& item) = 0;
    virtual Sequence<T>* InsertAt(const T& item, int index) = 0;
    virtual const T& operator[](int index) const = 0;

    // --- МЕТОДЫ С ОБЩЕЙ РЕАЛИЗАЦИЕЙ (БЕЗ = 0) ---
    ...
    // Map-Reduce (в нижнем регистре согласно вашему старому стилю)
